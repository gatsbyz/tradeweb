...
import { ExpressMiddleware, ServiceManager } from "./app";
...
export class Server {
  ...
  async start(silent = false): Promise<void> {
    const logger = silent
      ? (...args): void => log.trace(...args)
      : (...args): void => log.info(...args);
    ...
    const { expressMiddleware, expressControllers } = this.serviceManager;
    ...
    await Bluebird.fromCallback(
      (cb: any) => this.server && this.server.listen(this.config.port, cb)
    );
