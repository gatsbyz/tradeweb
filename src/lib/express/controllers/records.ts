...
export class Records {
  ...
  create(request): Record {
    const record = new CreateRecord(request.body);

    return this.services.records.create(Record.create(record));
  }
   ...
   * @returns {Record[]}
   */
  batchCreate(request): Record[] {
    const records = request.body.map((body) => new CreateRecord(body));
    return records.map((record: Record) =>
      this.services.records.create(Record.create(record))
    );
  }
