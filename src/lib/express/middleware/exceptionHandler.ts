...
export class ExceptionHandler {
  ...
  async handleJsonParseError(error, request, response, next): Promise<void> {
    if (error.type && error.type === "entity.parse.failed") {
      await this.services.responseBuilder.respond(
        response,
        new Err(
  ...
  async handleError(error, request, response, next): Promise<void> {
    ...
    if (error.stack) {
      log(`Unhandled error: ${error.stack}`);
    } else {
      log(`Unhandled error without stack: ${JSON.stringify(error)}`);
    }

    await this.services.responseBuilder.respond(
      response,
      new Err("Unhandled error occurred", HTTP_STATUS.INTERNAL_SERVER_ERROR)
    );
  }
  ...
  async handleNotFound(request, response): Promise<void> {
    ...
    await this.services.responseBuilder.respond(
      response,
      new Err(
        `URL: ${url} with method: ${method} is not a valid path`,
