...
const log = debug("responseBuilder");
...
export class ResponseBuilder {
  ...
  async respond(
    ...
    if (
      successCodeOverride &&
      !Object.values(HTTP_STATUS).includes(successCodeOverride)
    ) {
      log("successCodeOverride must be a valid HTTP code, ignoring");
      successCodeOverride = undefined;
    }

    if (
      failureCodeOverride &&
      !Object.values(HTTP_STATUS).includes(failureCodeOverride)
    ) {
      log("failureCodeOverride must be a valid HTTP code, ignoring");
      failureCodeOverride = undefined;
    }

    await Bluebird.try(() =>
      isFunction(dataPromise) ? dataPromise() : dataPromise
    )
      .then((data) => {
        // eslint-disable-next-line lodash/prefer-lodash-typecheck
  ...
  wrap(
    ...
    successCodeOverride =
      successCodeOverride || (controllerFunction as any).successCodeOverride;
    failureCodeOverride =
      failureCodeOverride || (controllerFunction as any).failureCodeOverride;
    ...
    return async (request, response, next): Promise<void> => {
      ...
      await Bluebird.try(() =>
        isFunction(controllerFunction)
          ? controllerFunction(request, response as any, nextCheck)
          : controllerFunction
      )
        ...
        .then(async (output) => {
          ...
          if (request.locals?.limitResult?.blockReason) {
            log.warn(
              `Overriding code: ${output.code} due to rate limit: ${request.locals?.limitResult?.blockReason} from IP: ${request.ip}`
            );
