...
const createFixture = () => {
  ...
  const config = {
    port: 9001,
    stripeWebhookSecret: "foo",
    slackSigningSecret: "foo",
  };
  const services = {
    limiters: {},
    responseBuilder: { wrap: (func) => func },
  } as any;
  const middleware = ServiceManager.buildExpressMiddleware(
    config as any,
    services
  );
  const controllers = ServiceManager.buildExpressControllers(
    config as any,
    services
  );
